...
# Author: xingjunjie
# Create Time: 29/11/2017 8:37 AM on PyCharm
...
class Model(object):
    def __init__(self, args, ntags, nwords, ntarwords=None, src_embedding=None,
        ...
        self.describe = "shared lstm with domain specific lstm, with mmd, model-3"
    ...
    def add_shared_lstm(self):
        with tf.device('/gpu:{:d}'.format(self.args.gpu_device)):
            ...
            self.src_lstm = tf.concat([self.src_after_shared, self.src_after_specific], axis=-1)
            self.target_lstm = tf.concat([self.target_after_shared, self.src_after_specific], axis=-1)

            with tf.variable_scope('src_lstm_linear'):
                W = tf.get_variable("W", shape=[4 * self.args.lstm_hidden, self.ntags],
                ...
                b = tf.get_variable("b", shape=[self.ntags], dtype=tf.float32,
                                    initializer=self.initializer, regularizer=self.l2_regularizer)
                ntime_steps = tf.shape(self.src_lstm)[1]
                output = tf.reshape(self.src_lstm, [-1, 4 * self.args.lstm_hidden])
            ...
            with tf.variable_scope('target_lstm_linear'):
                ...
                b = tf.get_variable("b", shape=[self.ntags], dtype=tf.float32,
                                    initializer=self.initializer, regularizer=self.l2_regularizer)
                ntime_steps = tf.shape(self.target_lstm)[1]
                output = tf.reshape(self.target_lstm, [-1, 4 * self.args.lstm_hidden])
                pred = tf.matmul(output, W) + b
